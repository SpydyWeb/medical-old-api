...
namespace WorkflowCore.Services
{
	public class WorkflowController : IWorkflowController
	{
		...
		{
			return this.StartWorkflow<object>(workflowId, version, data, reference);
		}

		public Task<string> StartWorkflow<TData>(string workflowId, TData data = null, string reference = null) where TData : class, new()
		{
			return StartWorkflow(workflowId, null, data, reference);
		}
		...
		public async Task<string> StartWorkflow<TData>(string workflowId, int? version, TData data = null, string reference = null) where TData : class, new()
		{
			...
			{
				await scope.ServiceProvider.GetRequiredService<IWorkflowMiddlewareRunner>().RunPreMiddleware(wf, def);
			}
		...
		public async Task PublishEvent(string eventName, string eventKey, object eventData, DateTime? effectiveDate = null)
		{
			...
			{
				@event.EventTime = effectiveDate.Value.ToUniversalTime();
			}
			else
			{
				@event.EventTime = _dateTimeProvider.UtcNow;
			}
			@event.EventData = eventData;
			@event.EventKey = eventKey;
			@event.EventName = eventName;
			@event.IsProcessed = false;
			string id = await _persistenceStore.CreateEvent(@event);
			await _queueProvider.QueueWork(id, QueueType.Event);
		}

		public async Task<bool> SuspendWorkflow(string workflowId)
		{
			if (!(await _lockProvider.AcquireLock(workflowId, default(CancellationToken))))
			{
				return false;
			}
			bool result;
		...
		{
			TWorkflow val = ActivatorUtilities.CreateInstance<TWorkflow>(_serviceProvider, Array.Empty<object>());
			_registry.RegisterWorkflow(val);
		}

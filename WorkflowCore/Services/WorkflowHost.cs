...
{
	public class WorkflowHost : IWorkflowHost, IWorkflowController, IActivityController, IHostedService, IDisposable
	{
		...
		public Task<string> StartWorkflow<TData>(string workflowId, int? version, TData data = null, string reference = null) where TData : class, new()
		{
		...
		{
			Activity activity = WorkflowActivity.StartHost();
			try
			{
				_shutdown = false;
				...
				foreach (IBackgroundTask backgroundTask in _backgroundTasks)
				{

...
namespace WorkflowCore.Services.BackgroundTasks
{
	internal class IndexConsumer : QueueConsumer, IBackgroundTask
	{
		...
		protected override async Task ProcessItem(string itemId, CancellationToken cancellationToken)
		{
			try
			{
				WorkflowInstance workflow = await FetchWorkflow(itemId);
			...
			{
				Logger.LogWarning(default(EventId), "Error indexing workfow - " + itemId + " - " + ex.Message);
				int num = 0;
				lock (_errorCounts)
				{
				...
				Logger.LogError(default(EventId), ex, "Unable to index workfow - " + itemId + " - " + ex.Message);
			}
		}

		private async Task<WorkflowInstance> FetchWorkflow(string id)
		{
			IPersistenceProvider store = _persistenceStorePool.Get();
			try
			{
